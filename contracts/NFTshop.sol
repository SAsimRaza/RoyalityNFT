...
contract NFTstore is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) public typeToRoyalityFees;
    ...
    function mint(uint256 _price, fileType _ftype) public payable {
        ...
        Item memory item = Item({ id: supply, currentOwner: msg.sender, price: _price, ftype: _ftype });

        uint256 royality = typeToRoyalityFees[uint256(_ftype)];
        ...
        if (msg.sender != governance) {
            ...
            (bool success2, ) = payable(governance).call{ value: (msg.value - royality) }("");
            require(success2);
        }
    ...
    function transferFrom(
        ...
        if (msg.value > 0) {
            uint256 royality = typeToRoyalityFees[uint256(idToItem[tokenId].ftype)];
    ...
    function setRoyality(fileType _ftype, uint256 value) external onlyOwner {
        typeToRoyalityFees[uint256(_ftype)] = value;
    }
